...
short* CAudioTimeSandPitchS::WavReadBuffer(short* buffer, unsigned long bufferSize)
{
	//16位的录音,单声道
	m_PCMSize = bufferSize/ 16 * 8;
	return buffer;
}
...
float* CAudioTimeSandPitchS::TimeScalingAndPitchShiftingRobot(int dst_freq, float dst_time, float* dataIn, int winSize)
{

	auto dataout = TimeScalingRobot(dataIn, winSize, 44100 / dst_freq, dst_time);
	return dataout;
}
float* CAudioTimeSandPitchS::TimeScalingAndPitchShiftingRobot(int dst_freq, float dst_time, short* dataIn, int winSize)
{
	float* dataout = TimeScalingRobot(dataIn, winSize, 44100 / dst_freq, dst_time);
	return dataout;
}
...
float* CAudioTimeSandPitchS::TimeScalingRobot(short* DataIn, int winSize, int hop, float scale)
{
	m_winSize = winSize;
	m_hop = hop;
	m_STFTOutRow = ((m_PCMSize - winSize) / m_hop) + 1;
...
complex** CAudioTimeSandPitchS::STFT(short* dataIn)
{
	complex **DataOut;
	DataOut = new complex*[m_STFTOutRow];
	for (int i = 0;i<m_STFTOutRow;i++)
